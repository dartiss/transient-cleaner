...
if ( ( ( ! empty( $_POST['Options'] ) ) || ( ! empty( $_POST['Upgrade'] ) ) || ( ! empty( $_POST['Clean'] ) ) ) && ( check_admin_referer( 'transient-cleaner-options', 'transient_cleaner_options_nonce' ) ) ) {
	...
	if ( isset( $_POST['clean_enable'] ) ) {
		$options['clean_enable'] = sanitize_text_field( $_POST['clean_enable'] );
	} else {
		$options['clean_enable'] = '';
	}
	if ( isset( $_POST['clean_optimize'] ) ) {
		$options['clean_optimize'] = sanitize_text_field( $_POST['clean_optimize'] );
	} else {
		$options['clean_optimize'] = '';
	}
	if ( isset( $_POST['upgrade_enable'] ) ) {
		$options['upgrade_enable'] = sanitize_text_field( $_POST['upgrade_enable'] );
	} else {
		$options['upgrade_enable'] = '';
	}
	if ( isset( $_POST['upgrade_optimize'] ) ) {
		$options['upgrade_optimize'] = sanitize_text_field( $_POST['upgrade_optimize'] );
	} else {
	...
	if ( ! empty( $_POST['Clean'] ) ) {
		$deleted = tc_transient_delete( false );
	}
	if ( ! empty( $_POST['Upgrade'] ) ) {
		$deleted = tc_transient_delete( true );
	}
...
if ( false !== $array ) {
	if ( isset( $array['records'] ) ) {
		$text = sprintf( __( '%d expired transient records were cleared on %s at %s.', 'artiss-transient-cleaner' ), $array['records'], gmdate( 'l, jS F Y', $array['timestamp'] ), gmdate( 'H:i', $array['timestamp'] ) );
	} else {
		$text = sprintf( __( 'Expired transient records were cleared on %s at %s.', 'artiss-transient-cleaner' ), gmdate( 'l, jS F Y', $array['timestamp'] ), gmdate( 'H:i', $array['timestamp'] ) );
	}
} else {
...
if ( false !== $array ) {
	if ( isset( $array['records'] ) ) {
		echo '<p>' . sprintf( __( 'All %d transient records were removed on %s at %s.', 'artiss-transient-cleaner' ), $array[ 'records' ], gmdate( 'l, jS F Y', $array['timestamp'] ), gndate( 'H:i', $array['timestamp'] ) ) . '</p>';
	} else {
		echo '<p>' . sprintf( __( 'All transient records removed on %s at %s.', 'artiss-transient-cleaner' ), gmdate( 'l, jS F Y', $array['timestamp'] ), gmdate( 'H:i', $array['timestamp'] ) ) . '</p>';
	}
}
?>
